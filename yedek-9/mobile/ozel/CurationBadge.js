...
export default function CurationBadge({
  ...
  const borderStyle = signal === 'premium' && !onDark ? { borderWidth: 1, borderColor: p.border } : null;

  return (
    <View style={[styles.base, mini && styles.mini, { backgroundColor: bg }, borderStyle, style]}>
      <Icon
        name={p.icon}
        size={mini ? 7 : 9}
        color={textColor}
      ...
      />
      <Text style={[styles.text, mini && styles.textMini, { color: textColor }]}>{label}</Text>
    </View>
  );
}
