...
export default function FriendsView({
  ...
  const [loading, setLoading] = useState(Boolean(fetchFriends) && initialFriends.length === 0);
  ...
  // Prop-driven kullanımda (fetchFriends yok), spinner'da kilitlenmeyi önle.
  useEffect(() => {
    if (!fetchFriends) {
      setLoading(false);
  ...
  }, [fetchFriends, initialFriends, initialMemories]);
