...
/**
 * HeroCard — Super Event büyük cinematic kart (280px+ min).
 *
 * data: {
 *   id, title, subtitle,                     // "Milano Design Week", tanıtım metni
 *   coverImage,
 *   dateRange,                               // "15-22 NİSAN"
 *   curationSignals,                         // ['super-event', 'trending']
 *   collectionStats: {                       // koleksiyon istatistikleri
 *     ritualCount, hostCount, totalSeats, takenSeats
 *   },
 *   liveStats: { currentViewers },
 *   _computed,
 * }
 */
export function HeroCard({ data, onPress }) {
  ...
  return (
    ...
    >
      ...
      >
        <LinearGradient
          colors={['rgba(10,10,10,0.2)', 'rgba(10,10,10,0.4)', 'rgba(10,10,10,0.95)']}
          locations={[0, 0.4, 1]}
          style={StyleSheet.absoluteFill}
        />
      </ImageBackground>
          ...
          <View style={styles.heroStats}>
            {stats.ritualCount != null && (
              <Text style={styles.heroStat}>
                <Text style={styles.heroStatBold}>{stats.ritualCount}</Text>
                {' ritüel'}
              </Text>
            )}
            {stats.hostCount != null && (
              <Text style={styles.heroStat}>
                <Text style={styles.heroStatBold}>{stats.hostCount}</Text>
                {' host'}
              </Text>
            )}
            {stats.totalSeats != null && (
              <Text style={styles.heroStat}>
                <Text style={styles.heroStatBold}>{stats.takenSeats || 0}</Text>
                {`/${stats.totalSeats} yer`}
              </Text>
            )}
          </View>
...
/**
 * FullCard — Editorial full-width kart (premium dinner, partnership, editor's pick).
 *
 * data: {
 *   id, title, host,
 *   coverImage,
 *   curationSignals,
 *   price: { kind, amount },               // { kind: 'paid', amount: '120€' }
 *   collectionLink,                         // "Milano Design Week'in parçası"
 *   curatorNote,                            // LOCAL'in notu
 *   meta: {                                 // 3-sütun info
 *     date, venue, seats: '3/12'
 *   },
 *   availability: { taken, total, waitlist },
 *   liveStats: { currentViewers },
 *   guests: [{ name, avatar }],             // opsiyonel özel misafirler
 *   footerNote,                             // footer sol taraf ("Dâhil: 7 yemek...")
 *   _computed,
 * }
 */
export function FullCard({ data, onPress, onRSVP, onJoinWaitlist }) {
  ...
  // CTA'yı duruma göre seç
  const ctaLabel = isWaitlist
    ? 'Bekleme Listesi'
    : signals.includes('invite-only') ? 'Başvur' : 'RSVP';
  ...
  return (
    ...
    >
      {/* Altın sol accent */}
      <LinearGradient
        colors={[colors.goldDeep, colors.gold, colors.goldLight || colors.gold]}
        style={styles.fullLeftAccent}
      />
      ...
      <View style={styles.fullCover}>
        ...
        <View style={styles.fullCoverTop}>
          <View style={styles.fullBadgeRow}>
            {signals.slice(0, 2).map((sig) => (
              <CurationBadge
                ...
                signal={sig}
                onDark
                overrideLabel={sig === 'limited' && computed.limitedText ? computed.limitedText : undefined}
              />
            ))}
          ...
          {data.price && (
            <PricePill
              kind={data.price.kind}
              label={data.price.amount}
            />
          )}
        </View>
        ...
        <View style={styles.fullCoverBottom}>
          ...
          {data.host && (
            <Text style={styles.fullHost} numberOfLines={1}>
              {data.host}
            </Text>
          )}
        </View>
      ...
      <View style={styles.fullBody}>
        ...
        {data.curatorNote && (
          <View style={styles.curatorNote}>
            ...
            <View style={styles.curatorNoteIcon}>
            ...
            <View style={styles.guestsAvatars}>
              {data.guests.slice(0, 3).map((g, i) => (
                <Image
                  key={i}
                  source={{ uri: g.avatar }}
                  style={[styles.guestAvatar, i > 0 && { marginLeft: -6 }]}
                />
              ))}
            </View>
            <View style={styles.guestsTextWrap}>
              <Text style={styles.guestsLabel}>ÖZEL MİSAFİRLER</Text>
              <Text style={styles.guestsText} numberOfLines={1}>
                {formatGuestNames(data.guests)}
              </Text>
            </View>
          </View>
        )}
        ...
        {computed.availText && (
          <LiveAvailabilityBar
            availText={computed.availText}
            percent={computed.availPercent}
            state={computed.availState}
            viewerStatus={computed.viewerStatus}
          />
        )}
...
function MetaItem({ icon, label, value, danger }) {
  return (
    <View style={styles.metaItem}>
      ...
      <Text
        style={[styles.metaValue, danger && { color: colors.live }]}
        numberOfLines={1}
      >
        {value}
      </Text>
