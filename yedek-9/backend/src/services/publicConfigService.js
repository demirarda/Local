...
export function getPublicConfig() {
  ...
  return {
    ...
    ritual: {
      ...
      feedback_freeze_0208: ritual.FEEDBACK_FREEZE_0208 === true,
      ...
      planned_max_ahead_d: ritual.PLANNED_MAX_AHEAD_D ?? 30,
      max_create_horizon_d: ritual.MAX_CREATE_HORIZON_D ?? ritual.PLANNED_MAX_AHEAD_D ?? 30,
      venue_raf_horizon_unlimited: ritual.VENUE_RAF_HORIZON_UNLIMITED !== false,
      ...
      self_rez_buffer_min: ritual.SELF_REZ_BUFFER_MIN ?? 30,
      door_set_user: ritual.DOOR_SET_USER || ['PUBLIC', 'APPROVAL', 'FRIENDS', 'SOLO'],
      door_set_business: ritual.DOOR_SET_BUSINESS || ['PUBLIC', 'APPROVAL'],
      forum_default_on: ritual.FORUM_DEFAULT_ON !== false,
      whole_window_visibility_default: ritual.WHOLE_WINDOW_VISIBILITY_DEFAULT || 'OFF',
      sub_fb_max: ritual.SUB_FB_MAX ?? 3,
      ven_event: {
    ...
    venue: {
      ...
      door_policy: venue.DOOR_POLICY || ['WALKIN_OPEN', 'SHELF_ONLY'],
      door_policy_default: venue.DOOR_POLICY_DEFAULT || 'WALKIN_OPEN',
      package_canonical_tiers: venue.PACKAGES_STUB?.CANONICAL_TIERS || ['OPEN', 'OPERATOR', 'LANDMARK'],
      slot_limit_removed: venue.PACKAGES_STUB?.SLOT_LIMIT_REMOVED === true,
      ...
      min_answers_per_obs: venue.MIN_ANSWERS_PER_OBS ?? 1,
      membership_vitrine_default: venue.MEMBERSHIP_VITRINE_DEFAULT !== false,
      seller_at_venue: venue.SELLER_AT_VENUE !== false,
      org_to_org: venue.ORG_TO_ORG !== false,
      seats: venue.PACKAGES_STUB?.SEATS || null,
      take_rate: venue.PACKAGES_STUB?.TAKE_RATE || null,
      announce_quota_mo: venue.PACKAGES_STUB?.ANNOUNCE_QUOTA_MO || null,
      badge_studio: venue.PACKAGES_STUB?.BADGE_STUDIO || null,
      price_eur: venue.PACKAGES_STUB?.PRICE_EUR || null,
      series_empty_sleep_w: venue.PACKAGES_STUB?.SERIES_EMPTY_SLEEP_W ?? 3,
      multi_branch_discount_visible: venue.PACKAGES_STUB?.MULTI_BRANCH_DISCOUNT_VISIBLE !== false,
      custom_totem_order_open: venue.PACKAGES_STUB?.CUSTOM_TOTEM_ORDER_OPEN !== false,
    },
    leads: {
    ...
    regular: {
      ...
      n: regular.N,
      min_seals: regular.MIN_SEALS ?? regular.N,
      window_d: regular.WINDOW_D,
      decay_d: regular.DECAY_D,
      venue_may_raise_only: regular.VENUE_MAY_RAISE_ONLY !== false,
      decay_d: regular.DECAY_D,
    ...
    chip: (() => {
      try {
        ...
        return {
          ...
          sets: chip.SETS,
          routes: chip.ROUTES,
          no_chips_for: chip.P2P_ENABLED === false ? ['p2p', 'p2host'] : ['p2host'],
          fiyat_open: true,
          min_chips_per_branch: Number(chip.MIN_CHIPS_PER_BRANCH ?? 4),
    ...
    zone: {
      spark_enabled: Boolean(zone.SPARK_ENABLED),
      trust_number: zone.TRUST_NUMBER === true,
      league_launch: zone.LEAGUE_LAUNCH !== false,
      door_public_only: zone.DOOR_PUBLIC_ONLY !== false,
      first_seal: [...(zone.FIRST_SEAL || ['GPS', 'ZONE_TOTEM_TAP'])],
      take_rate_z: Number(zone.TAKE_RATE_Z ?? 0.08),
      character_window_d: Number(zone.CHARACTER_WINDOW_D ?? 90),
      naming_shelf: zone.NAMING_SHELF !== false,
      league_prize: zone.LEAGUE_PRIZE || 'badge_fame',
      identity: ['aura', 'liveness', 'character'],
      public_coord_exact: LOCAL_CONFIG.p2c?.PUBLIC_COORD_EXACT === true,
    },
    search: {
    ...
    rs_display: {
      ...
      visibility: {
        ...
        placement_seals: rs.visibility?.PLACEMENT_SEALS ?? 5,
      },
      far_phase: rs.far?.PHASE ?? 0,
      w_cf: Number(rs.W_CF) || 0,
      no_peer: {
    ...
    fl_display: {
      ...
      fl1_full_weight: fl.FL1_FULL_WEIGHT ?? 1,
      metric: fl.METRIC || 'co_seal',
    },
    stubs: {
    ...
    totem: {
      static_qr_forbidden: LOCAL_CONFIG.totem?.STATIC_QR_FORBIDDEN !== false,
      modes: [...(LOCAL_CONFIG.totem?.MODES || ['NFC', 'ROTATING_CODE'])],
      rotating_code_ttl_s: LOCAL_CONFIG.totem?.ROTATING_CODE_TTL_S ?? 30,
      path_c: LOCAL_CONFIG.totem?.PATH_C || ['STAFF_DEVICE', 'TAP_POINT', 'FIGUR'],
      remote_deactivate: true,
    },
    window_tools: {
      types: [...(LOCAL_CONFIG.window_tools?.TYPES || ['photo', 'video', 'quote', 'music'])],
      camera_roll_first: LOCAL_CONFIG.window_tools?.CAMERA_ROLL_FIRST !== false,
    },
    life_joker: {
      per_days: LOCAL_CONFIG.penalties?.LIFE_JOKER?.PER_DAYS ?? 90,
      count: LOCAL_CONFIG.penalties?.LIFE_JOKER?.COUNT ?? 1,
    },
    terminology: {
      slot_retired: true,
      ui_noun: 'raf',
      raf_request_en: LOCAL_CONFIG.ritual?.RAF_REQUEST_EN || 'Create a Ritual Request',
      rezidan: 'REZİDAN',
      rezidan_en: 'RESIDENT',
    },
    p2c: {
      zone_min_people: LOCAL_CONFIG.p2c?.ZONE_MIN_PEOPLE ?? 4,
      zone_min_rituals: LOCAL_CONFIG.p2c?.ZONE_MIN_RITUALS ?? 2,
      home_never_zone: LOCAL_CONFIG.p2c?.HOME_NEVER_ZONE !== false,
    },
    bonds: {
      kinds: ['EKIP', 'REZIDAN', 'MENSUP'],
      chain_fren_cooldown_d: LOCAL_CONFIG.venue?.CHAIN_FREN_COOLDOWN_D ?? 90,
    },
    block: {
      symmetric_anonymous: true,
    },
  };
}
